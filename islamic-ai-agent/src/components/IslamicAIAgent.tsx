...
const IslamicAIAgent = () => {
  ...
  // Memoized Hijri date calculation - only recalculates when date changes
  const hijriDate = useMemo(() => {
    ...
    const gregorianMonth = today.getMonth() + 1; // JavaScript months are 0-based
    ...
    // Convert Julian Day to Hijri (Islamic calendar)
    // Using the standard algorithm for Hijri conversion
  ...
  }, [currentTime.toDateString()]); // Only recalculate when date changes
  ...
  const quickActions = [
    ...
    { icon: User, label: 'Ask Scholar', action: 'ask_scholar', color: 'bg-red-500' }
  ];
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // Try to get response from Islamic AI backend
      console.log('🔄 Sending request to Islamic AI backend:', currentInput);
      ...
      const fallbackResponse = {
        id: messages.length + 2,
        text: getIslamicResponse(currentInput),
        sender: 'agent',
        timestamp: new Date(),
        type: 'fallback'
      };
  ...
  const detectIntent = (message) => {
    const lowerMsg = message.toLowerCase();
    if (lowerMsg.includes('prayer') || lowerMsg.includes('salah')) return 'prayer';
    if (lowerMsg.includes('quran') || lowerMsg.includes('verse')) return 'quran';
    if (lowerMsg.includes('hadith') || lowerMsg.includes('prophet said')) return 'hadith';
    if (lowerMsg.includes('dua') || lowerMsg.includes('supplication')) return 'dua';
    if (lowerMsg.includes('halal') || lowerMsg.includes('haram')) return 'fiqh';
    return 'general';
  };

  const handleQuickAction = async (action) => {
    if (action === 'ask_scholar') {
      // Test the Scholar API
      setIsTyping(true);
      try {
        const response = await fetch('http://localhost:5002/api/scholar', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({
            message: 'Tell me about the importance of prayer in Islam',
            scholar_type: 'fiqh_scholar'
          })
        });

        if (response.ok) {
          const data = await response.json();
          const scholarMessage = {
            id: messages.length + 1,
            text: data.response,
            sender: 'agent',
            timestamp: new Date(),
        ...
        const errorMessage = {
          id: messages.length + 1,
          text: "I apologize, but I'm having trouble connecting to the Islamic scholars right now. Please try asking your question directly in the chat.",
          sender: 'agent',
          timestamp: new Date(),
          type: 'error'
        };
        ...
        {messages.map((message) => (
          ...
          >
            ...
            >
              ...
              {message.sender === 'agent' && message.type === 'ai_response' && (
                <div className="mb-2">
                  <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                    🤖 Enhanced AI Scholar Response
                  </span>
                </div>
              )}
              {message.sender === 'agent' && message.type === 'fallback' && (
                <div className="mb-2">
              ...
              {message.sender === 'agent' && message.type === 'scholar_response' && (
                <div className="mb-2">
                  <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800">
                    👨‍🏫 Islamic Scholar Response
                  </span>
                </div>
              )}
