...
""",
    ...
    def get_scholar_response(self, query: str, scholar_type: Optional[str] = None, user_gender: str = "not_specified", latitude: Optional[float] = None, longitude: Optional[float] = None) -> str:
        ...
        try:
            ...
            try:
                ...
                if loop.is_running():
                    # Check if we can use run_until_complete or similar
                    # In most web servers (Flask/ASCI), we might need to use a thread or future
                ...
                response_msg = asyncio.run(agent(query_msg))
            
            # Extract content
            response_content = response_msg.content if hasattr(response_msg, 'content') else str(response_msg)
            
            return f"""🎓 **Scholar Response Synthesized from Local Knowledge Base:**
...
*This specialized response was synthesized by our AI scholars from locally stored authentic documents.*"""
    ...
    def get_collaborative_response(self, query: str, user_gender: str = "not_specified") -> str:
        ...
        try:
            ...
            # 3. Direct Model Call
            try:
                response = model(conference_prompt)
                content = response.text if hasattr(response, 'text') else str(response)
            except Exception as e:
                # Basic fallback
                content = f"The scholarly panel encountered an error: {e}"
            
            return f"""🎓 **Consolidated Scholarly Conference Response (Local-First)**
