...
Processes documents from backend/knowledge/data/ and stores them in ChromaDB.
"""
...
# Configuration
# Configuration - Points to the module directory (backend/knowledge/)
...
def split_text(documents: List[Document]) -> List[Document]:
    ...
    # NEW: Metadata Enrichment for Window Retrieval
    # Group chunks by source to assign sequential indices
    doc_chunk_counters = {}
    for chunk in chunks:
    ...
    # Inject total chunk count per source for boundary checking
    for chunk in chunks:
        source = chunk.metadata.get("source", "unknown")
        chunk.metadata["total_chunks"] = doc_chunk_counters[source]

    print(f"Split into {len(chunks)} chunks with contextual metadata.")
    return chunks
...
def update_vector_db(chunks: List[Document]):
    ...
    try:
        ...
        # Collect ALL documents for BM25 mapping
        if os.path.exists(CHROMA_PATH):
            vector_db = Chroma(persist_directory=CHROMA_PATH, embedding_function=embeddings)
        ...
        if all_texts:
            print(f"Indexing {len(all_texts)} documents into discrete BM25 Engine...")
            ...
            print(f"✅ Fast BM25 discrete index saved to {BM25_PATH}")
    except Exception as e:
