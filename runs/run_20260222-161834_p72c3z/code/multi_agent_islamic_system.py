...
Please provide a comprehensive, balanced synthesis that incorporates the best insights from each specialist."""
    ...
    def get_scholar_response(self, query: str, scholar_type: str = None, user_gender: str = "not_specified") -> str:
...
*This specialized response was retrieved from locally stored authentic documents.*"""
            ...
            # Incorporate gender context into the query if relevant
            gender_prefix = ""
            if user_gender.lower() == 'female':
                gender_prefix = "[Context: The user is female. Please provide tailored guidance if applicable, such as Fiqh of Nisa or specific spiritual advice for women.] "
            elif user_gender.lower() == 'male':
                gender_prefix = "[Context: The user is male.] "
                
            query_msg = Msg(name="user", content=gender_prefix + query, role="user")
