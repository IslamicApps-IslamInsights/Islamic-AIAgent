...
def get_name_of_allah(query: str) -> str:
    ...
    """
    names = {
        "1": {"ar": "الرحمن", "en": "Ar-Rahman", "mean": "The Most Gracious", "virtue": "Reciting it regularly brings peace of heart and mindfulness of Allah's mercy."},
        "2": {"ar": "الرحيم", "en": "Ar-Rahim", "mean": "The Most Merciful", "virtue": "Allah bestows His special mercy upon those who recognize Him by this name."},
        "3": {"ar": "الملك", "en": "Al-Malik", "mean": "The King / The Sovereign", "virtue": "Helps in understanding the true nature of power and humility."},
        # Simplified for brevity, in a real system this would be a complete DB
    }
    
    for k, v in names.items():
        if query == k or query.lower() == v["en"].lower() or query in v["ar"]:
            return f"""💠 **99 Names of Allah (Asma-ul-Husna)**
...
*\"And to Allah belong the best names, so invoke Him by them.\" (7:180)*"""
    ...
    return "💡 Please search for names like 'Ar-Rahman', 'Al-Malik', or numbers 1-99. (Integration with full list in progress)."
...
def get_adhkar(category: str = "morning") -> str:
    ...
    adhkar = {
        "morning": {
            ...
            "trans": "Asbahna wa-asbahal-mulku lillahi...",
            "mean": "We have entered the morning and at this very time the whole kingdom belongs to Allah...",
            "virtue": "Protects the believer throughout the day."
        },
        "evening": {
            "text": "أَمْسَيْنَا وَأَمْسَى الْمُلْكُ لِلَّهِ...",
            "trans": "Amsayna wa-amsal-mulku lillahi...",
            "mean": "We have entered the evening...",
    ...
    if cat in adhkar:
        item = adhkar[cat]
        return f"""🤲 **{category.title()} Adhkar**
...
*Reference: Hisn al-Muslim*"""
    ...
    return "💡 Available categories: morning, evening. (More coming soon)."
...
def get_hajj_umrah_guidance(ritual: str) -> str:
    ...
    """
    rituals = {
        "ihram": "The sacred state of a pilgrim. One must make Niyyah (intention) and recite Talbiyah.",
        "tawaf": "Circling the Kaaba seven times counter-clockwise, starting from the Hajar al-Aswad.",
        "sai": "Walking seven times between the hills of Safa and Marwa.",
        "arafat": "The pinnacle of Hajj. Staying at the plain of Arafat from Dhuhr to Maghrib on 9th Dhul Hijjah."
    }
    
    r = ritual.lower()
    if r in rituals:
        return f"""🕋 **Hajj & Umrah Guide: {ritual.capitalize()}**

📍 **Instruction**:
{rituals[r]}

📖 **Evidence**: 
*\"And perform properly the Hajj and Umrah for Allah.\" (2:196)*

✅ **Status**: In-progress (Follow our mobile app for step-by-step tracking)."""
    
    return "💡 Search for rituals: Ihram, Tawaf, Sai, Arafat."
...
def check_halal_guidance(item: str) -> str:
    ...
    """
    ingredients = {
        "e120": "🔴 **Haram/Avoid**: Carmine/Cochineal. Derived from insects.",
        "e441": "🟡 **Doubtful**: Gelatine. Halal if from Zabihah animals, Haram if from pork.",
        "alcohol": "🔴 **Haram**: Intoxicants are strictly forbidden even in small amounts for consumption.",
        "lecithin": "🟢 **Halal/Doubtful**: Usually from Soy (Halal), but can be from animal fat (Doubtful)."
    }
...
🔍 **Recommendation**: Check for 'Halal Certified' logos (HMC, HFA, etc.) on the packaging.

*\"O mankind, eat from whatever is on earth [that is] lawful and good.\" (2:168)*"""
            
    return f"🔍 **Halal Guidance for '{item}'**: Looking for specific ingredient data. Generally, all plant-based items are Halal unless cross-contaminated. Avoid intoxicants and pork derivatives."
